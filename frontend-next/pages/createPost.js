...
export default function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    ...
    const categories = Array.from(formRef.current.querySelectorAll('input[name="category"]:checked'))
      .map(cb => cb.value);
    ...
    if (privacyLevel === "2") {
      selectedFollowers = Array.from(formRef.current.querySelectorAll('input[name="selected_followers"]:checked'))
    ...
    const postData = {
      ...
      content,
      categories,
      privacy_level: parseInt(privacyLevel),
