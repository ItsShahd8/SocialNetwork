document.addEventListener('DOMContentLoaded', () => {
    ...
    if (createPostForm) {
        createPostForm.addEventListener('submit', function (event) {
            ...
            // Get privacy level
            const privacyLevel = parseInt(document.querySelector('input[name="privacy_level"]:checked').value);

            // Get selected followers for private posts
            let selectedFollowers = [];
            if (privacyLevel === 2) {
                selectedFollowers = Array.from(document.querySelectorAll('input[name="selected_followers"]:checked'))
